...
def createModel(photos, labels):
    ...
    save('TestingPhotos.npy', test_photos)
    save('TestingLabels.npy', test_labels)

    test_photos = load('TestingPhotos.npy')
    test_labels = load('TestingLabels.npy')
    ...
    model = load('model.h5')
    pred = model.predict(test_photos[84].reshape(1, 200, 200, 1))
    print(pred.argmax())
